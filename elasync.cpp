...
void copy_opts_vec(std::vector<MiniDNN::Optimizer *> &from,
                   std::vector<MiniDNN::Optimizer *> &to) {
...
void MiniDNN::NetworkExecutor::run_elastic_async(
    int batch_size, int num_epochs, int rounds_per_epoch, int window,
    int probing_interval, int probing_duration, int m_0,
    struct timeval start_time, int seed, bool use_lock) {
  ...
  for (size_t i = 0; i < num_threads; ++i) {
    ...
    thread_local_networks[i]->set_output(new MultiClassEntropy(
  ...
  auto f = [&](int id) {
    ...
    if (id >= current_parallelism) {
      ...
      if (id == current_parallelism) {
        // sync_num_finished = 0;
        sync_point =
            new std::barrier(num_threads - current_parallelism, sync_point_cbk);
      ...
      while (!should_stop.test()) {
        ...
        thread_local_networks[id]->backprop(x_batches[batch_index],
                                            y_batches[batch_index]);
      ...
      while (true) {
        ...
        // Uncomment with not doing the optimal probing/save stuff
        // if (local_step >= num_epochs * rounds_per_epoch) {
        //   should_stop.test_and_set();
        //   break;
        // }
        ...
        if (tau < tau_threshold) {
          ...
          if (tauadaptstrat != "NONE" &&
              local_step > tau_sample_start * num_threads &&
              local_step < tau_sample_stop * num_threads) {
        ...
        if (epoch_step == rounds_per_epoch - 1) {
          ...
          time_per_epoch.push_back(now.tv_sec - start_time.tv_sec +
                                   (double)(now.tv_usec - start_time.tv_usec) /
                                       1000000);
  ...
  while ((curr_step = step.load()) < num_epochs * rounds_per_epoch) {
    ...
    if (loss_grads.empty()) {
      ...
      double m_trend = 0.0;
      if (m_exec_values.size() >=
          2) { /* We need at least two values to get differences */
      ...
      window_skew = loss_grads.back() * scalar_loss_grad +
                    loss_jitter * scalar_loss_jitter + m_trend * scalar_m_trend;
    ...
    for (int search_count = 0; search_count < 5; search_count++) {
      ...
      for (int it = 1; it <= 3; it++) {
        ...
        m_times.push_back((double)(probe_start.tv_sec - start_time.tv_sec) +
                          (double)(probe_start.tv_usec - start_time.tv_usec) /
                              1000000);
        should_stop.clear();
        ...
        std::cout << phase_number << ", " << search_count << ", "
                  << current_parallelism << ", " << loss << std::endl;
    ...
    for (int m = window_btm; m <= window_top; m += window_step) {
      ...
      double probe_elapsed =
          (double)(probe_end.tv_usec - probe_start.tv_usec) / 1000000;
      ...
      std::cout << phase_number << ", " << current_parallelism << ", " << loss
                << std::endl;
    ...
    std::cout << "# <execution> current_parallelism == " << current_parallelism
              << std::endl;
