...
void MiniDNN::NetworkExecutor::run_elastic_async(int batch_size, int num_epochs,
    ...
    auto f = [&](int id) {
        if (id >= current_parallelism) {
            ...
            while (true) {
                ...
                #else
                if (std::chrono::high_resolution_clock::now() - phase_starttime > 5s) {
                    break;
                }
                #endif
                ...
                // Ensure that the vectors have enough space for this new epoch data
                while (epoch >= local_losses_per_epoch.at(id).size())
                    local_losses_per_epoch.at(id).push_back(0);

                while (epoch >= local_rounds_per_epoch.at(id).size())
                    local_rounds_per_epoch.at(id).push_back(0);

                local_losses_per_epoch.at(id).at(epoch) += loss;
                local_rounds_per_epoch.at(id).at(epoch)++;
                latest_epoch = epoch;
    ...
    while (phase_number <= 20) {
        ...
        for (int search_count = 0; search_count < 5; search_count++) {
            ...
            for (int it = 1; it <= 3; it++) {
                ...
                std::cout << phase_number << ", " << search_count << ", " << current_parallelism
                          << ", " << loss << ", contr = " << num_contributors << std::endl;
        ...
        for (int m = window_btm; m <= window_top; m += window_step) {
            ...
            if (loss <= 0.0001) {
                std::cout << "# bug: loss <= 0.0001\n";
                std::cout << "#      loss_contributors = " << loss_contributors << "\n";
            }

#ifdef PROBING_TEST
            if (loss < best_loss && loss > 0.0001) { // Sometimes a bug made loss = 0
#else
            if (loss_diff < best_probe_delta_loss) {
#endif
                best_probe_delta_loss = loss_diff;
        ...
        phase_firststep = step;
        should_stop.clear();
        std::cout << "starting workers for execution...\n";
        workers.start_all();
        workers.wait_for_all();
        std::cout << "they finished.\n";
