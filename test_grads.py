...
for gr in grads:
    for t_i in range(NUM_REPS):
        with open(f"runtime_logs/linear-{gr}-#{t_i}.txt", "w") as ofile:
            # Test CIFAR10
            cmd = f"./cmake-build/mininn -a HEURISTIC -G {gr} -A LENET -D CIFAR10 -n 100 -e 700 -b 16 -l 0.005 -N nolock-linear-start100-grad{format(gr, '.5f')}-#{t_i}"
            print(f"({t_i}) {cmd}")
            subprocess.call(cmd.split(" "), stdout=ofile)

# for t_i in range(NUM_REPS):
#     with open(f"runtime_logs/linear-switch-#{t_i}.txt", "w") as ofile:
#         cmd = f"./cmake-build/mininn -a HEURISTIC -A LENET -D CIFAR10 -n 100 -e 300 -b 16 -l 0.005 -N linear-smart-switch-#{t_i}"
#         print(f"({t_i}) {cmd}")
#         subprocess.call(cmd.split(" "), stdout=ofile)
